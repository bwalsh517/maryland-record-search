...
(function (global) {
    ...
    class SM35Series extends BaseSeries {
        ...
        { number: 269, sr: "2660", dateRanges: [{ startYear: 1951, startMonth: 6, endYear: 1951, endMonth: 6, part: null, split: [{ start: "Montgomery", end: "Worcester" }] }], note: "Jun. MO-WO" }
            ...
            this.standardRecordsByNumber = {};

            for (const record of this.STANDARD_RECORDS) {
                this.standardRecordsByNumber[record.number] = record;
            }

            // SM35-1 through SM35-72 have real archive.org scans,
            // addressed by each record's own `sr` value rather than a
            // shared collection/prefix table (see STANDARD_RECORDS
            // above) - all 269 records have an sr value, but it's
            // only meaningful for building a URL within this range
            // (see archiveUrl() below). 73-269 have no archive.org
            // scan at all; BaseSeries.archiveUrl()'s default MSA
        ...
        archiveUrl(number) {
            ...
            const record = this.standardRecordsByNumber[number];
